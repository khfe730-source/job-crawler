...
- 운영 중 확인된 실제 RPD: **모델별** 20 (공식 문서 1,000과 다름)
- 쿼터가 모델별 독립 → `GEMINI_MODELS = [gemini-2.5-flash, gemini-2.5-flash-lite]`를 라운드로빈 호출하여 실효 한도 2배 활용
- RPM 보호: 호출 간 `AI_CALL_DELAY_SECONDS=2`초 sleep (라운드로빈이라 모델당 실효 4초 간격)
- RPD 보호: 회당 `MAX_AI_CALLS_PER_RUN=4`건 × 4회/일(6시간 간격) = 16건/일 (모델당 8건)
...
2026-04-20 10:00:05 [INFO] __main__: === 필터링 시작 | 대상 4개 (회당 상한 4, 모델 ['gemini-2.5-flash', 'gemini-2.5-flash-lite']) ===
2026-04-20 10:00:30 [INFO] __main__: === 필터링 완료 | AI 호출: 4, 매칭: 1, API 오류: 0 | 누적: ... ===
2026-04-20 10:00:30 [INFO] __main__: === 실행 완료, 종료 ===
```
...
# Gemini 모델 (라운드로빈으로 호출되어 모델별 RPD를 합산 활용)
GEMINI_MODELS = ["gemini-2.5-flash", "gemini-2.5-flash-lite"]

# AI 필터 레이트리밋 (Gemini 무료 티어 모델별 RPD 20 × 2모델 합산 운영)
AI_CALL_DELAY_SECONDS = 2.0   # 호출 간 딜레이 (라운드로빈이라 모델당 실효 4초 간격으로 RPM 보호)
MAX_AI_CALLS_PER_RUN = 4      # 1회 실행당 AI 호출 상한 (4회/일 cron × 4 = 16건/일, 모델당 8건)
```
