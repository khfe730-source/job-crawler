...
1. gamejob.co.kr 채용공고 목록/상세 페이지 크롤링
2. SQLite DB로 이미 처리한 공고 중복 제거
3. `EXCLUDED_KEYWORDS` 사전 검사 (API 호출 없이 제외)
4. Claude Haiku AI가 구직 조건 부합 여부 판단
5. 조건에 맞는 공고를 Slack Block Kit 카드로 발송 (선호 회사는 ⭐ 표시)
6. 3시간마다 자동 반복
